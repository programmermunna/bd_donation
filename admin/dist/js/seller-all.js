...
function make_show(items) {
  const sellers_wrapper = document.getElementById("sellers_wrapper");
  sellers_wrapper.innerHTML = "";
  items.map((seller, i) => {
    sellers_wrapper.innerHTML += `
  ...
  <td class="p-3 border whitespace-nowrap">
    <div class="text-center">${seller.phone}</div>
  </td>
  <td class="p-3 border">
    <div class="text-center w-36 mx-auto font-medium">${seller.address}</div>
  </td>
  <td class="p-3 border whitespace-nowrap">
...
<td class="p-3 border whitespace-nowrap">
  <div class="text-lg text-center">${seller.city}</div>
</td>
  <td class="p-3 border whitespace-nowrap">
    <div class="w-full flex_center gap-1">

    <a href="edit.html?id=${seller.id}">
...
function confirm_delete() {
  const index = sellers.findIndex((c) => c.id === delete_id.toString());
  index > -1 && sellers.splice(index, 1);
  make_show(sellers);
  setTimeout(() => {
    delete_popup.style.display = "none";
