...
const view_seller = sellers.find((c) => c.id === get_id);

const view_seller_form = document.getElementById("view_seller_form");

function edit_info_btn() {
  location.replace(`edit.html?id=${get_id}`);
}

view_seller_form.innerHTML = `
    <div>
    <b>Name</b>
    <p>${view_seller.name}</p>
    </div>
    <div>
    <b>Email</b>
    <p>${view_seller.email}</p>
    </div>
    <div>
    <b>Phone</b>
    <p>${view_seller.phone}</p>
    </div>
    <div>
    <b>Address</b>
    <p>${view_seller.address}</p>
    </div>
    <div>
    <b>Shop Name</b>
    <p>${view_seller.shop_name}</p>
    </div>
    <div>
    <b>Account Holder</b>
    <p>${view_seller.account_holder}</p>
    </div>
    <div>
    <b>Account Number</b>
    <p>${view_seller.account_number}</p>
    </div>
    <div>
    <b>Bank Name</b>
    <p>${view_seller.bank_name}</p>
    </div>
    <div>
    <b>Branch Name</b>
    <p>${view_seller.branch_name}</p>
    </div>
    <div>
    <b>City</b>
    <p>${view_seller.city}</p>
    </div>
    <div>
    <b>Profile Photo</b>
    <img
    ...
    height="80"
    class="rounded"
    src="${view_seller.image}"
    />
    </div>
`;

view_seller_form.addEventListener("submit", function (e) {
  e.preventDefault();

  //   Update function (1)

  // Then go to sellers page (2)
  location.replace("all-sellers.html");
});
